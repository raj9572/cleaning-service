...
export const createProduct = async (req, res) => {
    try {
        ...
        res.status(201).json(createResponse(201, product, "product create successfully"));

    } catch (error) {
        res.status(500).json(ErrorResponse({ message: "Server Error", error: error.message }));
    }
};
...
export const getProductById = async (req, res) => {
    try {
        ...
        if (!product) return res.status(404).json(ErrorResponse(404,"Product not found"));
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        res.status(200).json(createResponse(200,{},"product deleted succussfully"));
...
export const disableEnableProduct = async (req, res) => {
    try {
        ...
        res.status(200).json(createResponse(200, product, `Product is ${product.inStock ? "now in stock" : "not in stock"}`));
