...
const createCategory = async (req, res) => {
    ...
    if (!title || !key) {
        return res.status(400)
            .json(ErrorResponse(400, "all field is required"))
    }
...
const fetchAllCategoryansSubCategory = async (req, res) => {
        ...
        return res.status(200).json(createResponse(200,category,"category found"))
...
const fetchSubCategory = async (req, res) => {
     const {categoryId} = req.params
        ...
        return res.status(200).json(createResponse(200,subCategory,"subCategory found"))
        } catch (error) {
            return res.status(500).json(ErrorResponse(500,"internal server error"))
        }
}
